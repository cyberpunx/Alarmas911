...
class Empleados extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('personas_persona_id', 'required'),
			array('temporal, activo', 'numerical', 'integerOnly'=>true),
			array('personas_persona_id', 'length', 'max'=>11),
			array('funcion', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('empleado_id, personas_persona_id, funcion, temporal, activo', 'safe', 'on'=>'search'),
		);
	}
